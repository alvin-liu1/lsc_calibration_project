...
def create_falloff_map(rows, cols, falloff_at_edge):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            # [关键修改] 使用二次方或更陡峭的曲线来模拟鱼眼衰减
            # 线性插值可能不够陡，这里使用 ratio^2 模拟
            # 公式: Gain = 1.0 + (Target - 1.0) * ratio^2
            # 当 ratio=0(中心), Gain=1.0
            # 当 ratio=1(边缘), Gain=falloff_at_edge

            scaling_factor = ratio ** 2  # 平方让中间影响小，边缘影响大

            falloff_map[r, c] = 1.0 + (falloff_at_edge - 1.0) * scaling_factor
